...
export function SidebarLinks(props) {
  ...
  let sidebarBgColor = useColorModeValue("#bc24231c", "brand.200");
  ...
  const createLinks = (routes) => {
    return routes?.map((route, index) => {
      if (route.category) {
        ...
        return (
          <NavLink className={"block w-[90%] mx-auto"} key={index} to={route.path}>
            {route.separator && (
            ...
            {route.icon ? (
              <Box
                ...
                className={`py-[5px] px-[20px] ${openSidebar ? 'rounded-full' : 'rounded-tl-full rounded-bl-full'}`}
              >
                <HStack
                  spacing={
